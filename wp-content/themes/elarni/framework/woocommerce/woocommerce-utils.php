...
if ( ! function_exists( 'elearni_html_output' ) ) {

	function elearni_html_output( $html ) {

		return apply_filters( 'elearni_html_output', $html );
...
if( ! function_exists( 'elearni_shop_archive_page' ) ) {

	function elearni_shop_archive_page() {
...
if( ! function_exists( 'elearni_single_product' ) ) {

	function elearni_single_product( $id , $position ) {
...
if( ! function_exists( 'elearni_single_product_settings' ) ) {

	function elearni_single_product_settings ( $id ) {
...
if( ! function_exists( 'elearni_woo_remove_anonymous_object_action' ) ) {

	function elearni_woo_remove_anonymous_object_action( $tag, $class, $method, $priority = null ){
...
if( ! function_exists( 'elearni_woo_loop_column_class' ) ) {

	function elearni_woo_loop_column_class($columns) {

		$columns = intval( $columns );

		$options = elearni_shop_archive_page();
...
if(!function_exists('elearni_check_item_is_in_cart')) {
	function elearni_check_item_is_in_cart( $product_id ){
...
if( ! function_exists( 'elearni_single_product_nav' ) ) {
	function elearni_single_product_nav() {
		...
		$output .= '<div class="dt-sc-single-product-nav">';
			...
			if ( ! empty( $prev ) ) {
				...
				$output .= '<div class="dt-sc-single-product-nav-btn product-nav-prev">';
					$output .= '<a href="'.esc_url( $prev->get_permalink() ).'">'.esc_html__('Previous product', 'elearni').'<span class="product-nav-btn-icon"></span></a>';
			...
			$output .= '<a href="'.esc_url($shop_page_url).'" class="dt-sc-single-product-nav-back-btn"><span>'.esc_html__('Back to products', 'elearni').'</span></a>';

			if ( ! empty( $next ) ) {

				$output .= '<div class="dt-sc-single-product-nav-btn product-nav-next">';
					$output .= '<a href="'.esc_url( $next->get_permalink() ).'">'.esc_html__('Next product', 'elearni').'<span class="product-nav-btn-icon"></span></a>';
					$output .= '<div class="dt-sc-single-product-nav-intro-wrapper">';
						$output .= '<div class="product-nav-intro">';
							$output .= '<div class="product-nav-intro-image">';
...
if( ! function_exists( 'elearni_single_product_sociable_share_follow' ) ) {
	function elearni_single_product_sociable_share_follow($product_id, $share_follow_type, $social_icon_style_cls, $social_icon_radius_cls, $social_icon_inline_alignment_cls) {
	    ...
	    if($share_follow_type == 'share') {
			...
			$output .= '<div class="dt-sc-single-product-share-container '.esc_attr($social_icon_style_cls).' '.esc_attr($social_icon_radius_cls).' '.esc_attr($social_icon_inline_alignment_cls).'">';
				...
				$output .= '<a class="dt-sc-single-product-share-item-icon">'.esc_html__('Share', 'elearni').'</a>';
			...
			if( is_array( $sociable_flds ) && !empty( $sociable_flds ) ) {
				...
				if(!empty( $list )) {
					...
					$output .= '<div class="dt-sc-single-product-follow-container '.esc_attr($social_icon_style_cls).' '.esc_attr($social_icon_radius_cls).' '.esc_attr($social_icon_inline_alignment_cls).'">';
						$output .= '<a class="dt-sc-single-product-follow-item-icon">'.esc_html__('Follow', 'elearni').'</a>';
...
if ( ! function_exists( 'elearni_products_ajax_call' ) ) {

	function elearni_products_ajax_call() {
	...
	add_action( 'wp_ajax_elearni_products_ajax_call', 'elearni_products_ajax_call' );
	add_action( 'wp_ajax_nopriv_elearni_products_ajax_call', 'elearni_products_ajax_call' );

}

// Product Shortcode - Ajax Pagination

if ( ! function_exists( 'elearni_products_ajax_pagination' ) ) {

	function elearni_products_ajax_pagination($max_num_pages, $current_page, $post_per_page, $function_call, $output_div, $shortcode_attrs) {
...
if ( ! function_exists( 'elearni_products_sale_countdown_timer' ) ) {

	function elearni_products_sale_countdown_timer() {

		$product_template = elearni_woo_product_single_template_option();
	...
	add_action( 'woocommerce_single_product_summary', 'elearni_products_sale_countdown_timer', 15 );

}

// Product Shortcode - Ajax Call

if ( ! function_exists( 'elearni_size_guide_popup' ) ) {

	function elearni_size_guide_popup() {
			...
			if($product_id > -1) {
				...
				if($single_product_size_guides) {
					echo '<div class="dt-sc-size-guide-popup-container">';
						...
						echo '</div>';
						echo '<div class="dt-sc-size-guide-popup-close">'.esc_html__('Close','elearni').'</div>';
					echo '</div>';
				}		
	...
	add_action( 'wp_ajax_elearni_size_guide_popup', 'elearni_size_guide_popup' );
	add_action( 'wp_ajax_nopriv_elearni_size_guide_popup', 'elearni_size_guide_popup' );
