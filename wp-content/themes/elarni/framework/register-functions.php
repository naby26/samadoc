 ...
 * Theme support
 * --------------------------------------------------------------------------- */
if (!function_exists('elearni_features')) {

	function elearni_features() {
		...
		if ( function_exists( 'add_theme_support' ) ) {
			...
			add_image_size( 'elearni-blog-thumb', 150, 120, true  ); 	// blog - list
			add_image_size( 'elearni-blog-ii-column', 750, 500, true  ); 	// blog - ii column
			add_image_size( 'elearni-blog-iii-column', 570, 380, true  ); 	// blog - iii column
			add_image_size( 'elearni-blog-list', 600, 400, true  ); 	// blog - list
			add_image_size( 'elearni-blog-ii-column-masonry', 750 ); 	// blog - ii column masonry
			add_image_size( 'elearni-blog-iii-column-masonry', 540 ); 	// blog - iii column masonry

			add_image_size( 'elearni-event-list', 420, 336, true  ); 	// event-calendar - list
			add_image_size( 'elearni-event-single-type1', 773, 520, true  ); // event-calendar - single
			add_image_size( 'elearni-event-single-type4', 570, 460, true  ); // event-calendar - single
			add_image_size( 'elearni-event-single-type5', 746, 770, true  ); // event-calendar - single
			add_image_size( 'elearni-event-list2', 420, 275, true  ); 	// event-calendar - shortcode list
		}
		...
		register_nav_menus( array(
			'main-menu' => esc_html__('Main Menu', 'elearni'),
		) );
		...
		add_theme_support( 'editor-color-palette', array(
			array(
				'name'  => esc_html__( 'Primary Color', 'elearni' ),
				'slug'  => 'primary',
				'color' => $primary_color,
			),
			array(
				'name'  => esc_html__( 'Secondary Color', 'elearni' ),
				'slug'  => 'secondary',
				'color' => $secondary_color,
			),
			array(
				'name'  => esc_html__( 'Tertiary Color', 'elearni' ),
				'slug'  => 'tertiary',
				'color' => $tertiary_color,
			)
		));
	}
	add_action('after_setup_theme', 'elearni_features');
}

/* ---------------------------------------------------------------------------
 *	Under Construction
 * --------------------------------------------------------------------------- */
if( ! function_exists('elearni_under_construction') ){
	function elearni_under_construction(){
...
if( cs_get_option( 'enable-comingsoon' ) ):
	add_action('template_redirect', 'elearni_under_construction', 30);

	// getting shortcode css ----------------------
	add_action('wp_enqueue_scripts', 'elearni_rand_css', 101);
	function elearni_rand_css() {
		$id = cs_get_option( 'comingsoon-pageid' );
		if ( $id ) {
			$shortcodes_custom_css = get_post_meta( $id, '_wpb_shortcodes_custom_css', true );
			if ( ! empty( $shortcodes_custom_css ) ) {
				
				wp_register_style( 'vc-shortcodes-custom-'.$id, '', false, ELEARNI_THEME_VERSION, 'all' );	
 ...
 * --------------------------------------------------------------------------- */
if( !function_exists('elearni_wp_list_categories') ){
	function elearni_wp_list_categories( $output ){
	...
	add_filter('wp_list_categories', 'elearni_wp_list_categories');
}

/* ---------------------------------------------------------------------------
 * Filter to modify default list archive widget view
 * --------------------------------------------------------------------------- */
if( !function_exists('elearni_wp_list_archive') ){
	function elearni_wp_list_archive( $link_html,$url, $text, $format, $before, $after ) {
	...
	add_filter('get_archives_link', 'elearni_wp_list_archive', 10, 6);	
}

/* ---------------------------------------------------------------------------
 * Filter to execute shortcode inside contact form7
 * --------------------------------------------------------------------------- */
if( !function_exists('elearni_wpcf7_form_elements') ){
	function elearni_wpcf7_form_elements($form) {
		$form = do_shortcode( $form );
		return $form;
	}
	add_filter('wpcf7_form_elements', 'elearni_wpcf7_form_elements');
}

/* ---------------------------------------------------------------------------
 * Pagination for Blog and Portfolio
 * --------------------------------------------------------------------------- */
function elearni_pagination( $query = false, $load_more = false ){
	global $wp_query;
		...
		if( $load_more ){
			...
			if( $paged < $total ){
				$output .= '<div class="column one pager_wrapper pager_lm">';
					$output .= '<a class="pager_load_more button button_js" href="'. get_pagenum_link( $next ) .'">';
						...
						$output .= '<span class="button_label">'. esc_html__('Load more', 'elearni') .'</span>';
					$output .= '</a>';
				$output .= '</div>';
			}
			...
			$output .= '<div class="column one pager_wrapper">';
				...
				$args = array(
					...
					'mid_size'           => $mid_size,
					'prev_next'          => true,
					'prev_text'          => '<i class="fas fa-angle-double-left"></i>'.esc_html__('Prev', 'elearni'),
					'next_text'          => esc_html__('Next', 'elearni').'<i class="fas fa-angle-double-right"></i>',
					'type'               => 'list'
				);
...
function elearni_events_title() {
	...
	if( tribe_is_month() && !is_tax() ) { 
		$title = sprintf( esc_html__( 'Events for %s', 'elearni' ), date_i18n( 'F Y', strtotime( tribe_get_month_view_date() ) ) );
	} elseif( class_exists('Tribe__Events__Pro__Main') && tribe_is_week() )  {
		$title = sprintf( esc_html__('Events for week of %s', 'elearni'), date_i18n( $date_format, strtotime( tribe_get_first_week_day($wp_query->get('start_date') ) ) ) );
	} elseif( class_exists('Tribe__Events__Pro__Main') && tribe_is_day() ) {
		$title = esc_html__( 'Events for', 'elearni' ) . ' ' . date_i18n( $date_format, strtotime( $wp_query->get('start_date') ) );
	} elseif( class_exists('Tribe__Events__Pro__Main') && (tribe_is_map() || tribe_is_photo()) ) {
		if( tribe_is_past() ) {
			$title = esc_html__( 'Past Events', 'elearni' );
		} else {
			$title = esc_html__( 'Upcoming Events', 'elearni' );
		}
		...
		$title = esc_html__('Tag Archives','elearni');
	}
	return $title;
}
 ...
 * Excerpt
 * --------------------------------------------------------------------------- */
function elearni_excerpt($limit = NULL) {
 ...
 * --------------------------------------------------------------------------- */
function elearni_wp_kses($content) {
 ...
 */
function elearni_before_after_widget ( $content ) {
	$allowed_html = array(
		'aside' => array(
			'id'    => array(),
			'class' => array()
 ...
 */
function elearni_widget_title( $content ) {
 ...
 * --------------------------------------------------------------------------- */
if(!function_exists('elearni_hex2rgb')) {

	function elearni_hex2rgb($hex) {
 ...
 * Theme Comment Style
 * --------------------------------------------------------------------------- */
function elearni_comment_style( $comment, $args, $depth ) {
	...
	case 'trackback':
		...
		echo "<p>";
		esc_html_e('Pingback:', 'elearni');
		comment_author_link();
		edit_comment_link(esc_html__('Edit', 'elearni'), ' ', '');
		echo "</p>";
		break;
	...
	case '':
		...
		echo '  <div class="comment-body">';
					...
					if ($comment->comment_approved == '0') :
						esc_html_e('Your comment is awaiting moderation.', 'elearni');
					endif;
					edit_comment_link(esc_html__('Edit', 'elearni'));
		echo '	</div>';
		echo '	<div class="reply">';
		echo 		comment_reply_link(array_merge($args, array('reply_text' => esc_html__('Reply', 'elearni'), 'depth' => $depth, 'max_depth' => $args['max_depth'])));
		echo '	</div>';
		echo '</section>';
 ...
 * --------------------------------------------------------------------------- */
function elearni_get_page_permalink_by_its_template( $template ) {
	$permalink = '#';
 ...
 * Theme show sidebar
 * --------------------------------------------------------------------------- */
function elearni_show_sidebar( $type, $id, $position = 'right' ) {
 ...
 * --------------------------------------------------------------------------- */
function elearni_global_variables($variable = '') {
...
/* ---------------------------------------------------------------------------
 * Walker Page for elearni_new_wp_page_menu
 * --------------------------------------------------------------------------- */
class ELEARNI_Walker_Page extends Walker_Page {
 ...
 * --------------------------------------------------------------------------- */
add_filter('upload_mimes', 'elearni_upload_mimes');
function elearni_upload_mimes( $existing_mimes = array() ){
...
* Whitelist Associate
* --------------------------------------------------------------------------- */
if ( ! function_exists( 'elearni_array_whitelist_assoc' ) ) {
	function elearni_array_whitelist_assoc( Array $array1, Array $array2 ) {
		if ( func_num_args() > 2 ) {
			$args = func_get_args();
			array_shift( $args );
...
* Post Type Support
* --------------------------------------------------------------------------- */
add_filter( 'fw_ext_page_builder_supported_post_types', 'elearni_limit_post_types_support', 1 );
if (!function_exists('elearni_limit_post_types_support')) {

	function elearni_limit_post_types_support( $all_post_types ) {
		$white_listed_post_types = array( '' ); //allowed custom post type names
		$post_types              = elearni_array_whitelist_assoc( $all_post_types, $white_listed_post_types );
		return $post_types;
	}
}
...
// Custom Widgets                    -
// -----------------------------------------
function elearni_custom_widgets() {
