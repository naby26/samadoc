...
jQuery(document).ready(function($){
	...
	//Apply Layout
	if($.cookie("elearni_layout") == "boxed"){
		$("ul.layout-picker li a").removeAttr("class");
		$("ul.layout-picker li a[id='"+$.cookie("elearni_layout")+"']").addClass("selected");
		$("div#pattern-holder").removeAttr("style");

		$i = ($.cookie("elearni_pattern")) ? $.cookie("elearni_pattern")  : 'pattern1';
		...
		$("ul.pattern-picker a[id="+$.cookie("elearni_pattern")+"]").addClass('selected');
	} else if($.cookie("elearni_layout") == "fullwidth"){
		$("ul.layout-picker li a").removeAttr("class");
		$("ul.layout-picker li a[id='"+$.cookie("elearni_layout")+"']").addClass("selected");
		$("div#pattern-holder").removeAttr("style");
	...
	$("ul.layout-picker a").on('click', function(e){
		...
		$.cookie("elearni_layout", $this.attr("id"), { path: '/' });

		if( $.cookie("elearni_layout") === "boxed") {
			$("body").removeClass("layout-wide").addClass("layout-boxed");
			$("div#pattern-holder").slideDown();
			
			if( $.cookie("elearni_pattern") == null ){
				$("ul.pattern-picker a:first").addClass('selected');
				$.cookie("elearni_pattern","pattern1",{ path: '/' });
			}else{
				$("ul.pattern-picker a[id="+$.cookie("elearni_pattern")+"]").addClass('selected');
				$img = dttheme_urls.theme_base_url + "/images/patterns/"+$.cookie("elearni_pattern")+".jpg";
	...
	$("ul.pattern-picker a").on('click', function(e){
		if($.cookie("elearni_layout") == "boxed"){
			...
			$("ul.pattern-picker a").removeAttr("class");
			$this.addClass("selected");
			$.cookie("elearni_pattern", $this.attr("id"), { path: '/' });
			$img = dttheme_urls.theme_base_url + "/images/patterns/"+$.cookie("elearni_pattern")+".jpg";
			$('body').css('background-image', 'url('+$img+')');
		}
		e.preventDefault();
