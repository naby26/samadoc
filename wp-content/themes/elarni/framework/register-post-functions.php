...
class elearni_post_functions {
	...
	function __construct() {
		...
		add_action( 'wp_ajax_elearni_post_rating_like', array( $this, 'elearni_post_rating_like' ) );
		add_action( 'wp_ajax_nopriv_elearni_post_rating_like', array( $this, 'elearni_post_rating_like' ) );

		add_filter( 'post_class', array( $this, 'elearni_add_remove_post_class' ), 10, 1 );

		add_action( 'wp_ajax_elearni_ajax_load_more_post', array( $this, 'elearni_ajax_load_more_post' ) );
		add_action( 'wp_ajax_nopriv_elearni_ajax_load_more_post', array( $this, 'elearni_ajax_load_more_post' ) );
	}

	function elearni_post_meta_fields( $ajax = false, $page_ID = 0 ) {
	...
	static function elearni_post_view_count($pid = '') {
	...
	static function elearni_post_like_count($pid = '') {
	...
	function elearni_post_rating_like() {
		...
		if ( wp_verify_nonce( $nonce, 'rating-nonce' ) && $postid > 0 ) {
			...
			if( isset( $_COOKIE['arr_pids'] ) ) {
				...
				if( in_array( $postid, explode(',', $_COOKIE['arr_pids']) ) ) {
					...
					$out = esc_html__('Already', 'elearni');
			...
			$out = esc_html__('Security check', 'elearni');
		}
	...
	function elearni_add_remove_post_class( $classes ) {
	...
	function elearni_ajax_load_more_post() {
		...
		if(wp_verify_nonce( $nonce, 'blog-nonce' )) {
			...
			if($the_query->have_posts()):
				...
				if( empty($meta) ) {
					$obj = new elearni_post_functions;
					$meta = $obj->elearni_post_meta_fields(true);
				}
				...
				while( $the_query->have_posts() ):
					...
					<div class="<?php echo esc_attr($temp_class);?>">
						<article id="post-<?php the_ID();?>" <?php post_class( array( 'blog-entry', 'format-'.$format ) );?>><?php
							...
							$template = apply_filters( 'elearni_blog_archive_template', 'framework/templates/archive-blog-entry.php' );
							...
							elearni_get_template( $template, $template_args );
				...
				<h2><?php esc_html_e('Nothing Found.', 'elearni'); ?></h2>
				<p><?php esc_html_e('Apologies, but no results were found for the requested archive.', 'elearni'); ?></p><?php
			endif;
