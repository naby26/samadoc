 ...
 * --------------------------------------------------------------------------- */
add_action('wp_enqueue_scripts', 'elearni_enqueue_scripts');
function elearni_enqueue_scripts() {
	...
	if( cs_get_option('enable-cookie-consent') == "true" ) {
		wp_enqueue_script('elearni-cookie-js', get_theme_file_uri('/framework/js/cookieconsent.js'), array(), false, true);
	}

	wp_enqueue_script('elearni-popup-js', get_theme_file_uri('/framework/js/magnific/jquery.magnific-popup.min.js'), array(), false, true);
	...
	if( function_exists( 'is_woocommerce' ) ){
		wp_enqueue_script('elearni-woocommerce', get_theme_file_uri('/framework/js/woocommerce.js'), array(), false, true);
	}
	wp_enqueue_script('elearni-jqcustom', get_theme_file_uri('/framework/js/custom.js'), array(), false, true);
	...
	wp_localize_script('jquery', 'dttheme_urls', array(
		'theme_base_url' => esc_js(ELEARNI_THEME_URI),
		'framework_base_url' => esc_js(ELEARNI_THEME_URI).'/framework/',
		'ajaxurl' => esc_url( admin_url('admin-ajax.php') ),
		'url' => esc_url( get_site_url() ),
		'isRTL' => esc_js($rtl),
		'loadingbar' => esc_js($loadingbar),
		'advOptions' => esc_html__('Show Advanced Options', 'elearni'),
	...
	if( isset($picker) ) {
		...
		wp_enqueue_script('elearni-jqcpanel', get_theme_file_uri('/framework/js/controlpanel.js'),array(),false,true);
	}
}
...
* --------------------------------------------------------------------------- */
function elearni_scripts_custom() {
	
	$enable_custom_js = (int) get_theme_mod( 'enable-custom-js', elearni_defaults('enable-custom-js') );
	$custom_js = get_theme_mod( 'custom-js', '');
	
	if( !empty( $enable_custom_js ) && !empty( $custom_js ) ){
		wp_add_inline_script('elearni-jqcustom', elearni_wp_kses(stripslashes($custom_js)) ,'after');
	}
}
add_action('wp_enqueue_scripts', 'elearni_scripts_custom', 100);

/* ---------------------------------------------------------------------------
 * Loading Theme Styles
 * --------------------------------------------------------------------------- */
add_action( 'admin_enqueue_scripts', 'elearni_jq_migration', 0 );
add_action( 'wp_enqueue_scripts', 'elearni_jq_migration', 150 );
function elearni_jq_migration() {
	wp_enqueue_script('jquery-migrate');
}

add_action( 'wp_enqueue_scripts', 'elearni_enqueue_styles', 101 );
function elearni_enqueue_styles() {

	// site icons ---------------------------------------------------------------
	if ( ! has_site_icon() ):
		$url = ELEARNI_THEME_URI . "/images/favicon.ico";
		echo "<link href='$url' rel='shortcut icon' type='image/x-icon' />\n";		
	endif;

	// wp_enqueue_style ---------------------------------------------------------------
	wp_enqueue_style( 'elearni', get_stylesheet_uri(), false, ELEARNI_THEME_VERSION, 'all' );

	wp_enqueue_style( 'elearni-base',		  get_theme_file_uri('/css/base.css'), false, ELEARNI_THEME_VERSION, 'all' );
	wp_enqueue_style( 'elearni-grid', 		  get_theme_file_uri('/css/grid.css'), false, ELEARNI_THEME_VERSION, 'all' );
	wp_enqueue_style( 'elearni-widget', 	  get_theme_file_uri('/css/widget.css'), false, ELEARNI_THEME_VERSION, 'all' );
	wp_enqueue_style( 'elearni-layout', 	  get_theme_file_uri('/css/layout.css'), false, ELEARNI_THEME_VERSION, 'all' );
	wp_enqueue_style( 'elearni-blog',	      get_theme_file_uri('/css/blog.css'), false, ELEARNI_THEME_VERSION, 'all' );
	wp_enqueue_style( 'elearni-contact',	  get_theme_file_uri('/css/contact.css'), false, ELEARNI_THEME_VERSION, 'all' );
	wp_enqueue_style( 'elearni-custom-class', get_theme_file_uri('/css/custom-class.css'), false, ELEARNI_THEME_VERSION, 'all' );
	wp_enqueue_style( 'elearni-browsers', 	  get_theme_file_uri('/css/browsers.css'), false, ELEARNI_THEME_VERSION, 'all' );

	wp_enqueue_style( 'prettyphoto',	get_theme_file_uri('/css/prettyPhoto.css'), false, ELEARNI_THEME_VERSION, 'all' );
	...
	if( cs_get_option( 'enable-comingsoon' ) )
		wp_enqueue_style("elearni-comingsoon",  get_theme_file_uri("/css/comingsoon.css"), false, ELEARNI_THEME_VERSION, 'all' );

	// notfound css
	if ( is_404() )
		wp_enqueue_style("elearni-notfound",	get_theme_file_uri("/css/notfound.css"), false, ELEARNI_THEME_VERSION, 'all' );

	// loader css
	$loadingbar = cs_get_option( 'use-site-loader' );
	if( !empty( $loadingbar ) )
		wp_enqueue_style("elearni-loader", 		get_theme_file_uri("/css/loaders.css"), false, ELEARNI_THEME_VERSION, 'all' );

	// woocommerce css
	if( function_exists( 'is_woocommerce' ) ):
		wp_enqueue_style( 'elearni-woo', 				get_theme_file_uri('/css/woocommerce.css'), 'woocommerce-general-css', ELEARNI_THEME_VERSION, 'all' );
		wp_enqueue_style( 'elearni-woo-default', 		get_theme_file_uri('/css/woocommerce/woocommerce-default.css'), false, ELEARNI_THEME_VERSION, 'all' );
		wp_enqueue_style( 'elearni-woo-hovers', 		get_theme_file_uri('/css/woocommerce/woocommerce-hovers.css'), false, ELEARNI_THEME_VERSION, 'all' );
		wp_enqueue_style( 'elearni-woo-custom',    get_theme_file_uri('/css/woocommerce/woocommerce-custom.css'), false, ELEARNI_THEME_VERSION, 'all' );
	endif;

	// tribe-events -------------------------------------------------------------------
	wp_enqueue_style( 'elearni-customevent', 		get_theme_file_uri('/tribe-events/custom.css'), false, ELEARNI_THEME_VERSION, 'all' );
	
	// cookie-consent -----------------------------------------------------------------
	if( cs_get_option('enable-cookie-consent') == "true" ) {
		wp_enqueue_style( 'elearni-cookie-css', 		get_theme_file_uri('/css/cookieconsent.css'), false, ELEARNI_THEME_VERSION, 'all' );
	}

	wp_enqueue_style( 'elearni-popup-css', 			get_theme_file_uri('/framework/js/magnific/magnific-popup.css'), false, ELEARNI_THEME_VERSION, 'all' );

	wp_enqueue_style( 'bxslider-css', 						get_theme_file_uri('/css/jquery.bxslider.min.css'), false, ELEARNI_THEME_VERSION, 'all' );

	// blog single css ---------------------------------------------------------------------
	wp_enqueue_style( 'elearni-custom', 			get_theme_file_uri('/css/blog-single.css'), false, ELEARNI_THEME_VERSION, 'all' );

	// jquery scripts --------------------------------------------
	wp_enqueue_script('modernizr-custom', 	get_theme_file_uri('/framework/js/modernizr.custom.js'), array('jquery'));

	// rtl ----------------------------------------------------------------------------
	if(is_rtl()) wp_enqueue_style('elearni-rtl', 	get_theme_file_uri('/css/rtl.css'), false, ELEARNI_THEME_VERSION, 'all' );
	
	// gutenberg css ---------------------------------------------------------------------
	wp_enqueue_style( 'elearni-gutenberg', get_theme_file_uri('/css/gutenberg.css'), false, ELEARNI_THEME_VERSION, 'all' );
	...
	if( !empty( $primary_color ) ) {
		...
		$rgba = elearni_hex2rgb( $primary_color );
		$rgba = implode(',', $rgba);
		...
		$css .= '.dt-sc-icon-box.type3.circle:hover .icon-wrapper{ box-shadow:0px 0px 0px 7px rgba('.$rgba.', 0.4) }'."\n";	
		$css .= '.process .dt-sc-timeline-item:hover .dt-sc-timeline-image, .process .dt-sc-timeline-item.slick-current .dt-sc-timeline-image{ box-shadow:0px 0px 0px 7px '.$primary_color.' }'."\n";	
		$css .= '.carousel_items.testimonial_carousel .dt-sc-testimonial.type1 .dt-sc-testimonial-author span:before{ box-shadow:0px 0px 0px 7px inset '.$primary_color.' }'."\n";	
		$css .= '.dt-sc-icon-box.type3.circle:hover .icon-wrapper{ border-color:rgba('.$rgba.', 0.4) }'."\n";	
	...
	if( !empty( $secondary_color ) ) {
		...
		$rgba = elearni_hex2rgb( $secondary_color );
		$rgba = implode(',', $rgba);
	...
	if( !empty( $tertiary_color ) ) {
		...
		$rgba = elearni_hex2rgb( $tertiary_color );
		$rgba = implode(',', $rgba);
	...
	if( !empty($primary_color) && !empty($secondary_color) ) {
		...
		$css .= '.dt-sc-button.gradient-button, .dtlms-course-category-item.type8:hover .dtlms-category-total-items a, .dt-sc-icon-box.type3.circle:hover .icon-wrapper span, .contact-form button[type=submit], .dtlms-button, .type6.dtlms-courselist-item-wrapper .dtlms-courselist-details .dtlms-coursedetail-cart-details .added_to_cart, body #buddypress input[type="reset"], body #buddypress input[type="submit"], input[type="submit"], button, input[type="button"], input[type="reset"], .gradient-accordion.dt-sc-toggle-frame-set .dt-sc-toggle-frame h5.dt-sc-toggle-accordion.active, .gradient-accordion.dt-sc-toggle-group-set .dt-sc-toggle-frame h5.dt-sc-toggle.active, .dt-sc-team.team-profile .dt-sc-team-thumb::before, .dtlms-instructor-item.type2.team-profile .dtlms-instructor-item-thumb::before{
			background-image: -webkit-linear-gradient(left, '.$primary_color.', '.$secondary_color.');
			background-image: -moz-linear-gradient(left, '.$primary_color.', '.$secondary_color.');
			background-image: -o-linear-gradient(left, '.$primary_color.', '.$secondary_color.');
			background-image: -ms-linear-gradient(left, '.$primary_color.', '.$secondary_color.');
			background-image: linear-gradient(to right, '.$primary_color.', '.$secondary_color.');
		}';

		$css .= '.pic-section a.gradient-video-icon{
			background-image: -webkit-linear-gradient(bottom, '.$primary_color.', '.$secondary_color.');
			background-image: -moz-linear-gradient(bottom, '.$primary_color.', '.$secondary_color.');
			background-image: -o-linear-gradient(bottom, '.$primary_color.', '.$secondary_color.');
			background-image: -ms-linear-gradient(bottom, '.$primary_color.', '.$secondary_color.');
			background-image: linear-gradient(to top, '.$primary_color.', '.$secondary_color.');
		}';

		$css .= '.gradient-progress .dt-sc-progress .dt-sc-bar{
			background-image: -webkit-linear-gradient(left, '.$secondary_color.', '.$primary_color.');
			background-image: -moz-linear-gradient(left, '.$secondary_color.', '.$primary_color.');
			background-image: -o-linear-gradient(left, '.$secondary_color.', '.$primary_color.');
			background-image: -ms-linear-gradient(left, '.$secondary_color.', '.$primary_color.');
			background-image: linear-gradient(to right, '.$secondary_color.', '.$primary_color.');
		}';

	}

	wp_add_inline_style( 'elearni-custom', $css );

	$fonts = cs_get_option('custom_font_fields');
	if( isset( $fonts ) && count( $fonts ) > 0 ){
		wp_add_inline_style('elearni-custom', elearni_styles_custom_font() );
	}
}

add_action( 'wp_enqueue_scripts', 'elearni_enqueue_custom_inline', 999 );
if ( ! function_exists( 'elearni_enqueue_custom_inline' ) ) {
	function elearni_enqueue_custom_inline() {
		wp_register_style( 'elearni-custom-inline', '', array(), ELEARNI_THEME_VERSION, 'all' );
	}
}
 ...
 * --------------------------------------------------------------------------- */
function elearni_styles_custom_font() {
	$out = '';
 ...
 * --------------------------------------------------------------------------- */
function elearni_body_classes( $classes ) {
	
	// layout
	$classes[] 		= 	'layout-'. get_theme_mod( 'site-layout', elearni_defaults('site-layout') );
...
add_filter( 'body_class', 'elearni_body_classes' );?>