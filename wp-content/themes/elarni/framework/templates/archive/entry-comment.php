...
<?php
 	if( ! post_password_required() ) {
		if( $post_Style == 'dt-sc-simple-style' || $post_Style == 'dt-sc-simple-withbg-style' || $post_Style == 'dt-sc-overflow-style' ):
			comments_popup_link( esc_html__('Add Comment', 'elearni'), esc_html__('1 Comment', 'elearni'), esc_html__('% Comments', 'elearni'), '', esc_html__('Comments Off', 'elearni'));
		...
		elseif( $post_Style == 'dt-sc-classic-style' ):
			echo '<i class="zmdi zmdi-lock zmdi-hc-fw"> </i>'.esc_html__('Enter your password to view comments.', 'elearni');
		else:
			echo '<i class="fas fa-ban"> </i>'.esc_html__('Enter your password to view comments.', 'elearni');
		endif;
	}?>