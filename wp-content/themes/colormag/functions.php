...
function afficher_doc(){
	$con=mysqli_connect("localhost","root","","samadoc");
    $req=mysqli_query($con,"SELECT name FROM test");
	while($table=mysqli_fetch_array($req)){
		?>
	<a href="http://localhost/samadoc/disicode/sd_repertoire/<?php echo $table["name"];?>">Visualiser</a>
	<?php
	echo $table["name"]."<br>";
	}

}
add_shortcode("afficher","afficher_doc");